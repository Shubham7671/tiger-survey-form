...
const QuestionsAndAnswers = (props) => {
    ...
    const incrementHandler = () => {
        ...
        if ((props.selected === "Multi-select" && id === 3) || (props.selected === "Single select" && id === 1)) {
            setAddNpublish(true);
        }
    }
                    ...
                    {Object.keys(answers).map((key) => (
                        <div key={key} className="input-group">
                            ...
                            <div className="input-group-append">
                                <span key={key} onClick={incrementHandler} className={`input-group-text ${addNpublish ? "disable" : ""}`}>+</span>
                                <span accessKey={key} onClick={decrementHandler} className="input-group-text">-</span>
                            </div>
                        </div>
                    ))}
                </div>
